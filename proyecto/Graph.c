...
void G_Best_Route(  Station* origen, Station* destino, DLL* stations, DLL* roots, char path[], size_t len)
{
	...
	while( !(DLL_IsEmpty(queue)) )				///Mientras la cola no esté vacia se genera todo el algoritmo
	{
		...
		Station* u = ( Station* )c;					///Se castea a un objeto City
		...
		{
			Station* n = ( Station* )nodesList[i];	///Se castea cada vecino a un objeto City
			...
			{
				Roots* a = ( Roots* )nodesList2[j];	///Se castea cada arista a un objeto Axis

 ...
 *
 * @param origen        Referencia a un objeto City con el que comenzaremos nustro algoritmo.
 * @param destino       Referencia a un objeto City con el que terminaremos nuestro algoritmo.
 * @param cities        Referencia a una DLL donde se almacenan todos nuestros nodos representados por ciudades.
 * @param axis          Referencia a una DLL donde se almacenan todas nustras aristas junto con sus pesos.
...
void G_Best_Route(City* origen, City* destino, DLL* cities, DLL* axis, char path[], size_t len)
{
	...
	Add_Cities( cities, vertices);							///Se agregan todas los nodos (ciudades) al mapa
	...
	while( !(DLL_IsEmpty(queue)) )				///Mientras la cola no esté vacia se genera todo el algoritmo
	{
		...
		City* u = ( City* )c;					///Se castea a un objeto City
		...
		{
			City* n = ( City* )nodesList[i];	///Se castea cada vecino a un objeto City

			size_t listLen2 = DLL_Len( axis );
			void* nodesList2[ listLen2 ];
			DLL_GetInformation( axis, nodesList2);  ///Se obtinen todas las aristas del grafo

			for( size_t j=0; j < listLen2; j++ )	///Se itera sobre todas las aristas del grafo
			{
				Axis* a = ( Axis* )nodesList2[j];	///Se castea cada arista a un objeto Axis
		...
		City* n2 = (City*)nodesList[best_neighbor]; 	///Se busca en la tabla y se castea al mejor vecino
...
{
	size_t listLen = DLL_Len( cities );

	void* nodesList[ listLen ];

	DLL_GetInformation( cities, nodesList);
